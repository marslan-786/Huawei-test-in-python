...
# --- IMPORT SOLVER (Must be in same folder) ---
try:
...
async def burst_wait(page, seconds, step_name):
    ...
    for i in range(frames):
        ...
        try: await page.screenshot(path=f"{CAPTURE_DIR}/{filename}")
        except: pass
            ...
            if await inp.count() > 0:
                ...
                if await get_code.count() > 0:
                    ...
                    for i in range(15): # Try for 15 seconds
                        ...
                        for frame in page.frames:
                            try:
                                if await frame.get_by_text("swap 2 tiles", exact=False).count() > 0:
                                    ...
                                    # CALL SOLVER FUNCTION (from captcha_solver.py)
                                    session_id = f"sess_{int(time.time())}"
                                    # Pass 'logger' so it prints to web console
                        ...
                        if captcha_solved_status: break
                        await asyncio.sleep(5) # Check every second
                    
                    if not captcha_solved_status:
                         log_msg("❓ Captcha did not appear or was not detected.")
