...
async def dashboard():
    ...
    <body>
        ...
        <div class="section">
            ...
            <div id="gallery-wrapper" class="hidden"><div class="gallery" id="gallery"></div></div>
        </div>
        ...
        <div class="section">
            ...
            <div id="video-wrapper" class="hidden"><video id="v-player" controls autoplay loop></video></div>
        </div>

        <script>
            let showPics = false;
            let showAI = false;
            function startBot() { fetch('/start', {method: 'POST'}); logUpdate(">>> COMMAND SENT: START BOT"); }
            function refreshData() {
                fetch('/status').then(r=>r.json()).then(d=>{
                    document.getElementById('logs').innerHTML = d.logs.map(l=>`<div class="log-entry">${l}</div>`).join('');
                    if (showPics) document.getElementById('gallery').innerHTML = d.images.map(i=>`<a href="${i}" target="_blank"><img src="${i}"></a>`).join('');
                    if (showAI) document.getElementById('ai-gallery').innerHTML = d.ai_images.length === 0 ? "<p>No AI images found yet.</p>" : d.ai_images.map(i=>`<a href="${i}" target="_blank"><img src="${i}"></a>`).join('');
                });
            }
            function togglePictures() {
                ...
                if (showPics) { wrapper.classList.remove('hidden'); btn.classList.remove('btn-view'); btn.classList.add('btn-close'); btn.innerText = "❌ CLOSE GALLERY"; refreshData(); }
                else { wrapper.classList.add('hidden'); btn.classList.remove('btn-close'); btn.classList.add('btn-view'); btn.innerText = "📸 VIEW CAPTURES"; document.getElementById('gallery').innerHTML = ""; }
            }
            function toggleAI() {
                showAI = !showAI;
                const wrapper = document.getElementById('ai-wrapper');
                const btn = document.getElementById('btn-ai');
                if (showAI) { wrapper.classList.remove('hidden'); btn.innerText = "❌ CLOSE AI VIEW"; refreshData(); }
                else { wrapper.classList.add('hidden'); btn.innerText = "🧠 VIEW AI ANALYSIS"; document.getElementById('ai-gallery').innerHTML = ""; }
            }
            function makeVideo() {
                var status = document.getElementById('video-status');
                var wrapper = document.getElementById('video-wrapper');
                status.innerText = "⏳ PROCESSING VIDEO... PLEASE WAIT..."; status.style.color = "yellow"; wrapper.classList.add('hidden');
                fetch('/generate_video', {method: 'POST'}).then(r=>r.json()).then(d=>{
                    if(d.status === "done") {
                        status.innerText = "✅ VIDEO READY!"; status.style.color = "#00e676"; wrapper.classList.remove('hidden');
                        var player = document.getElementById('v-player'); player.src = "/captures/proof.mp4?t=" + Date.now(); player.load(); player.play();
                    } else { status.innerText = "❌ ERROR: " + d.error; status.style.color = "red"; }
                });
            }
            function logUpdate(msg) { var logs = document.getElementById('logs'); logs.innerHTML = "<div class='log-entry'>[" + new Date().toLocaleTimeString() + "] " + msg + "</div>" + logs.innerHTML; }
...
async def get_status():
    ...
    ai_files = []
    if os.path.exists(f"{CAPTURE_DIR}/ai_solved_preview.jpg"): ai_files.append(f"{CAPTURE_DIR}/ai_solved_preview.jpg")
    ...
    ai_urls = []
    for f in ai_files:
        if "debug_ai" in f: ai_urls.append(f"/captures/debug_ai/{os.path.basename(f)}")
        else: ai_urls.append(f"/captures/{os.path.basename(f)}")

    return JSONResponse({"logs": logs, "images": flow_urls, "ai_images": ai_urls})
    ...
    try:
        with imageio.get_writer(VIDEO_PATH, fps=15, format='FFMPEG', quality=8) as writer:
            for filename in files:
                try: img = imageio.imread(filename); writer.append_data(img)
                except: continue
        return {"status": "done"}
    ...
    try:
        ...
        if box:
            ...
            await page.evaluate(f"""
                var dot = document.createElement('div'); dot.style.position = 'absolute'; left = '{x}px'; top = '{y}px';
                dot.style.width = '20px'; height = '20px'; background = 'red'; dot.style.borderRadius = '50%'; zIndex = '999999'; dot.style.border = '2px solid yellow';
...
async def burst_wait(page, seconds, step_name):
    ...
    for i in range(frames):
        ts = datetime.now().strftime("%H%M%S%f"); filename = f"{ts}_{step_name}.jpg"
        await page.screenshot(path=f"{CAPTURE_DIR}/{filename}"); await asyncio.sleep(0.1)
...
async def run_russia_flow():
    ...
    async with async_playwright() as p:
        ...
        browser = await p.chromium.launch(headless=True, args=["--disable-blink-features=AutomationControlled", "--no-sandbox"], proxy=PROXY_CONFIG)
        ...
        try:
            ...
            # ... (Rest of flow same as previous code) ...
            # I am keeping it short here to fit character limit, just paste the previous flow logic here.
            # Only change is the solver call below:
            ...
            await page.mouse.move(200, 500); await page.mouse.down(); await page.mouse.move(200, 800, steps=20); await page.mouse.up()
            ...
            if await hk_selector.count() > 0:
                ...
                if await page.locator("input").count() > 0:
                    ...
                    if await target.count() > 0: await visual_tap(page, target, "Select_Russia"); await burst_wait(page, 3, "08_russia_set")
            
            inp = page.locator("input[type='tel']").first
            if await inp.count() == 0: inp = page.locator("input").first
            if await inp.count() > 0:
                await visual_tap(page, inp, "Input")
                for char in current_number: await page.keyboard.type(char); await asyncio.sleep(0.1)
                await page.touchscreen.tap(350, 100); await burst_wait(page, 1, "09_ready")
                ...
                if await get_code.count() > 0:
                    ...
                    if len(page.frames) > 1:
                        ...
                        # PASS LOGGER HERE!
                        await solve_captcha(page, "SESSION_X", log_msg) 
                        await burst_wait(page, 5, "11_post_swap")
                    else: log_msg("❓ No Captcha Frame.")
